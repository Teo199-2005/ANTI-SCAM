...
export async function POST(req: NextRequest) {
  ...
  try {
    backendRes = await fetch(`${BACKEND}/auth/google-complete`, {
      method: "POST",
  ...
  const payload = (await backendRes.json()) as {
    success: boolean;
    message?: string;
    data?: { token: string; user: unknown; referral_trial?: unknown };
  };
  ...
  const res = NextResponse.json({
    success: true,
    data: { user: payload.data.user, referral_trial: payload.data.referral_trial },
  });
