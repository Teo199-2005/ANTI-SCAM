...
import { BadgeCheck, Shield, Sparkles, X } from "lucide-react";
  ...
  return createPortal(
    ...
    >
      <div className="relative flex min-h-[100dvh] items-center justify-center px-4 py-8 sm:px-6 sm:py-10">
        <button
          type="button"
          className="absolute inset-0 min-h-full bg-[#0d1f3c]/65 backdrop-blur-[3px] transition-opacity"
        ...
        <div
          ...
          className="relative z-10 w-full max-w-md overflow-hidden rounded-2xl border border-slate-200/95 bg-white shadow-[0_25px_50px_-12px_rgba(13,31,60,0.35)] ring-1 ring-black/5"
        >
          ...
          <div className="flex items-start justify-between gap-3 border-b border-slate-100 bg-white px-4 py-3.5 sm:px-5 sm:py-4">
            ...
            <button
              type="button"
              onClick={onClose}
              className="shrink-0 rounded-xl border border-slate-200 bg-white p-2 text-slate-600 transition hover:bg-slate-50 hover:text-slate-900"
              aria-label="Close"
            >
          ...
          <div className="space-y-4 bg-white px-4 py-5 sm:px-6 sm:py-6">
