...
type Props = {
  ...
  /** Renders inner content only (inside `ResortLandingAboutMapBand` left panel). */
  embedded?: boolean;
};

function AboutContent({ about, amenities }: Pick<Props, "about" | "amenities">) {
...
export function ResortLandingStoryGallery({ about, amenities, surface = "odd", embedded = false }: Props) {
  const body = about.body?.trim() ?? "";
  const hasBody = Boolean(body);
  const hasAmenities = amenities.length > 0;
  ...
  if (embedded) {
    return <AboutContent about={about} amenities={amenities} />;
  }
  ...
  return (
    <section id="about" className={cn("resort-landing-section scroll-mt-24 border-t border-zinc-200/70", band)}>
      <ScrollReveal className="resort-landing-container" direction="up" delayMs={40}>
        <AboutContent about={about} amenities={amenities} />
      </ScrollReveal>
    </section>
  );
}
